...
*(Everything below reflects the current state: a fully functional traditional backend, plus the first layer of the AI/RAG pipeline — local embedding generation and vector storage. Retrieval, grounded Q&A, and the agentic reviewer are the next phases.)*
...
| Testing | pytest, 36 tests, real Postgres + real Chroma in integration tests |
...
Highlights:
- **Tenant isolation** enforced at the repository layer — every query scoped by `org_id`, empirically verified with a cross-tenant access test (not just asserted). The same principle now extends to vector search: Chroma's metadata filter is treated as a performance optimization, not a security boundary — verified with an automated test proving one tenant's query returns zero results from another tenant's data, even against a single shared vector collection.
- **Idempotent ingestion** — re-uploading the same document doesn't duplicate chunks; verified by reprocessing an already-completed document and confirming deterministic, byte-identical output. Embedding generation carries the same guarantee: re-embedding a document version is proven, via automated test, to leave the vector count and vector IDs unchanged rather than duplicating or orphaning data.
...
- **Local, open-source embeddings over a hosted API** — `bge-small-en-v1.5` runs entirely on CPU, no external API cost or network dependency for the embedding step, chosen over a marginally larger model for the retrieval-quality-per-resource trade-off. CPU-only PyTorch is installed explicitly rather than relying on the default GPU-enabled build, avoiding roughly 1.8GB of unused CUDA dependencies.
...
- **Phase B** (AI/RAG layer) — in progress
  ...
  - B2 — RAG retrieval & grounded Q&A endpoint (hybrid search: vector + keyword)
...
└── services/      # business logic (parsing, chunking, ingestion, embedding, vector store, queue)
